...
export class WorkbenchAuthoring extends Context.Service<
	WorkbenchAuthoring,
...
export const WorkbenchAuthoringLive = Layer.effect(
	WorkbenchAuthoring,
	Effect.gen(function* () {
		...
		const loadSavedTable = Effect.fn("Workbench.WorkbenchAuthoring.loadSavedTable")(function* (
			assetPath: string
		) {
		...
		const loadLiveTable = Effect.fn("Workbench.WorkbenchAuthoring.loadLiveTable")(function* (
			objectPath: string
		) {
		...
		const refreshCatalog = Effect.fn("Workbench.WorkbenchAuthoring.refreshCatalog")(function* (
			projectRoot: string
		) {
		...
		const configuredTable = Effect.fn("Workbench.WorkbenchAuthoring.configuredTable")(
			function* () {
		...
		const configuredCatalog = Effect.fn("Workbench.WorkbenchAuthoring.configuredCatalog")(
			function* () {
		...
		const chooseTable = Effect.fn("Workbench.WorkbenchAuthoring.chooseTable")(function* () {
			const choice = yield* dialog.chooseFile({
				filters: [{ extensions: ["uasset"], name: "Unreal saved assets" }],
		...
		const openCatalogTable = Effect.fn("Workbench.WorkbenchAuthoring.openCatalogTable")(
		...
		const beginSession = Effect.fn("Workbench.WorkbenchAuthoring.beginSession")(function* (
			objectPath: string
		) {
		...
		const redoSession = Effect.fn("Workbench.WorkbenchAuthoring.redoSession")(function* (
			sessionId: string
		) {
			if (Option.isNone(sessions)) {
				return sessionFailure({ message: "UE_SHED_PROJECT_ROOT is not configured" });
		...
		const applySession = Effect.fn("Workbench.WorkbenchAuthoring.applySession")(function* (
			sessionId: string
		) {
			if (Option.isNone(sessions)) {
				return sessionFailure({ message: "UE_SHED_PROJECT_ROOT is not configured" });
		...
		const reconcileSession = Effect.fn("Workbench.WorkbenchAuthoring.reconcileSession")(
			function* (sessionId: string) {
				...
				return yield* Effect.gen(function* () {
					...
					Effect.catch((cause) =>
						invalidateConnectionOnLiveFailure(cause).pipe(
							Effect.as(sessionFailure(cause))
						)
					)
				);
			}
		...
		const saveSession = Effect.fn("Workbench.WorkbenchAuthoring.saveSession")(function* (
			sessionId: string
		) {
			if (Option.isNone(sessions)) {
				return sessionFailure({ message: "UE_SHED_PROJECT_ROOT is not configured" });
...
): Layer.Layer<WorkbenchAuthoring> {
	return Layer.succeed(WorkbenchAuthoring, WorkbenchAuthoring.of(service));
}
