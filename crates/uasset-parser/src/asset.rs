...
pub const ED_GRAPH_CLASS: &str = "/Script/Engine.EdGraph";
const UE57_BLUEPRINT_PACKAGE_VERSIONS: &[i32] = &[1012, 1013, 1017, 1018];
...
///
/// Blueprint node subclasses keep the conventional `K2Node` prefix even when supplied by a game
/// or plugin module. Comment nodes use the shared engine `EdGraphNode_Comment` class and participate
/// in the same graph node arrays, so they are decoded through the same base node representation.
pub fn is_blueprint_graph_node_class(class_path: &str) -> bool {
...
/// Returns whether a package uses a saved revision covered by the UE 5.7 Blueprint graph decoder.
///
/// These are package serialization revisions observed in assets that the current UE 5.7 editor
/// loads. This is intentionally not a promise of cross-engine Blueprint compatibility.
#[must_use]
pub fn supports_blueprint_graph_package_version(versions: &crate::version::VersionContext) -> bool {
    versions.ue4 == crate::version::VersionContext::LATEST_SUPPORTED_UE4
        && UE57_BLUEPRINT_PACKAGE_VERSIONS.contains(&versions.ue5)
}
...
pub enum DecodedAsset {
    ...
    BlueprintGraphNode(DecodedBlueprintGraphNode),
}

/// A UE 5.7 Blueprint graph node with its shared native pin stream decoded.
///
/// Node subclasses may append their own native payload after the base pin array. That evidence is
/// retained in `tail` so supporting the common graph topology never hides subclass-specific gaps.
#[derive(Clone, Debug, PartialEq)]
pub struct DecodedBlueprintGraphNode {
    pub object_path: ObjectPath,
    pub class_path: ObjectPath,
    pub object_guid: Option<Guid>,
    pub properties: PropertyStream,
    pub pins: Vec<Option<EdGraphPin>>,
    pub tail: Span,
}

#[derive(Clone, Debug, PartialEq)]
pub struct EdGraphPin {
    pub owning_node: PackageIndex,
    pub id: Guid,
    pub name: NameRef,
    pub friendly_name: TextValue,
    pub source_index: i32,
    pub tooltip: String,
    pub direction: EdGraphPinDirection,
    pub pin_type: EdGraphPinType,
    pub default_value: String,
    pub autogenerated_default_value: String,
    pub default_object: PackageIndex,
    pub default_text_value: TextValue,
    pub linked_to: Vec<Option<EdGraphPinReference>>,
    pub sub_pins: Vec<Option<EdGraphPinReference>>,
    pub parent_pin: Option<EdGraphPinReference>,
    pub reference_pass_through_connection: Option<EdGraphPinReference>,
    pub persistent_guid: Guid,
    pub flags: u32,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EdGraphPinDirection {
    Input,
    Output,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EdGraphPinContainerType {
    None,
    Array,
    Set,
    Map,
...
pub struct EdGraphPinType {
    pub category: NameRef,
    pub subcategory: NameRef,
    pub subcategory_object: PackageIndex,
    pub container_type: EdGraphPinContainerType,
    pub value_type: Option<EdGraphTerminalType>,
    pub is_reference: bool,
    pub is_weak_pointer: bool,
    pub member_reference: EdGraphMemberReference,
    pub is_const: bool,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EdGraphTerminalType {
    pub category: NameRef,
    pub subcategory: NameRef,
    pub subcategory_object: PackageIndex,
    pub is_const: bool,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EdGraphMemberReference {
    pub parent: PackageIndex,
    pub name: NameRef,
    pub guid: Guid,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EdGraphPinReference {
    pub owning_node: PackageIndex,
    pub pin_id: Guid,
}
...
#[derive(Clone, Copy, Debug, Default)]
pub struct BlueprintGraphNodeDecoder;

impl AssetDecoder for BlueprintGraphNodeDecoder {
    fn supports(&self, class_path: &ObjectPath) -> bool {
        is_blueprint_graph_node_class(class_path.as_str())
    }
    ...
    fn decode(
        ...
        if !self.supports(class_path) {
            return Err(AssetError::new(
                AssetErrorKind::UnsupportedFormat,
                format!("unsupported Blueprint graph node class {class_path}"),
            ));
        }

        decode_saved_blueprint_graph_node(export, context).map(DecodedAsset::BlueprintGraphNode)
    }
}

/// Decodes the shared native `UEdGraphNode` payload for an export that a saved graph references in
/// its `Nodes` array. The graph membership is the capability proof, allowing arbitrary engine,
/// plugin, and project node subclasses without guessing from their class names.
pub fn decode_saved_blueprint_graph_node(
    export: &Export,
    context: &AssetDecodeContext<'_>,
) -> Result<DecodedBlueprintGraphNode, AssetError> {
    ...
    let versions = &context.package.summary.versions;
    if !supports_blueprint_graph_package_version(versions) {
        return Err(AssetError::new(
            AssetErrorKind::UnsupportedVersion,
            format!(
                "Blueprint graph pin decoding supports the UE 5.7-loadable saved package revisions (UE4 {}, UE5 {:?}); {} uses UE4 {}, UE5 {}",
                crate::version::VersionContext::LATEST_SUPPORTED_UE4,
                UE57_BLUEPRINT_PACKAGE_VERSIONS,
                export.object_path,
                versions.ue4,
                versions.ue5
            ),
        ...
        .contains(crate::version::PackageFlags::FILTER_EDITOR_ONLY)
    {
        return Err(AssetError::new(
            AssetErrorKind::UnsupportedCapability,
            format!(
                "Blueprint graph pins are editor-only and were filtered from {}",
                export.object_path
            ),
        ));
    }
    ...
    let (properties, mut reader) = decode_uobject_properties(export, context)?;
    let object_guid = consume_inline_object_guid_footer(&mut reader, &export.object_path)?;
    let pins = read_ed_graph_pin_array(&mut reader, &export.object_path)?;
    consume_empty_editable_user_pin_array(&mut reader, &class_path, &export.object_path)?;
    let tail = Span::new(reader.tell(), reader.remaining()).map_err(AssetError::from)?;

    Ok(DecodedBlueprintGraphNode {
        object_path: export.object_path.clone(),
        class_path,
        object_guid,
        properties,
...
fn consume_empty_editable_user_pin_array(
    reader: &mut crate::archive::Reader<'_>,
    class_path: &ObjectPath,
    object_path: &ObjectPath,
) -> Result<(), AssetError> {
    let class_name = class_path.as_str().rsplit('.').next();
    if !matches!(
        class_name,
        Some(
            "K2Node_Event"
    ...
    // These exact current-engine classes derive from UK2Node_EditablePinBase. Its additional saved
    // value is `TArray<FUserPinInfo>`. Consume the common empty case; preserve a non-empty array as
    // subclass evidence until FUserPinInfo is modeled.
    let mut probe = reader.clone();
    let count = probe.read_count(&format!("{object_path}.UserDefinedPins.Count"))?;
    if count == 0 {
        *reader = probe;
    }
    Ok(())
...
fn read_ed_graph_pin_array(
    reader: &mut crate::archive::Reader<'_>,
    object_path: &ObjectPath,
) -> Result<Vec<Option<EdGraphPin>>, AssetError> {
    let path = format!("{object_path}.Pins");
    let count = reader.read_count(&format!("{path}.Count"))?;
    let capacity =
        reader.checked_vec_capacity::<Option<EdGraphPin>>(count, 4, &format!("{path}.Count"))?;
    let mut pins = Vec::with_capacity(capacity);
    for index in 0..count {
        pins.push(read_ed_graph_pin(reader, &format!("{path}[{index}]"))?);
    }
    Ok(pins)
}

fn read_ed_graph_pin(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<Option<EdGraphPin>, AssetError> {
    let Some(reference) = read_ed_graph_pin_reference(reader, &format!("{path}.Reference"))? else {
        return Ok(None);
    };

    let owning_node = read_package_index(reader, &format!("{path}.OwningNode"))?;
    let id = reader.read_guid(&format!("{path}.PinId"))?;
    if reference.owning_node != owning_node || reference.pin_id != id {
        return Err(AssetError::new(
            AssetErrorKind::MalformedData,
            format!("{path} owner reference and serialized pin identity disagree"),
        ));
    }

    let name = reader.read_name_ref(&format!("{path}.PinName"))?;
    let friendly_name = read_ue57_text(reader, &format!("{path}.PinFriendlyName"))?;
    let source_index = reader.read_i32(&format!("{path}.SourceIndex"))?;
    let tooltip = read_blueprint_pin_string(reader, &format!("{path}.PinToolTip"))?;
    let direction = match reader.read_u8(&format!("{path}.Direction"))? {
        0 => EdGraphPinDirection::Input,
        1 => EdGraphPinDirection::Output,
        value => {
            return Err(AssetError::new(
                AssetErrorKind::MalformedData,
                format!("{path}.Direction has invalid UE 5.7 value {value}"),
            ));
        }
    };
    let pin_type = read_ed_graph_pin_type(reader, &format!("{path}.PinType"))?;
    let default_value = read_blueprint_pin_string(reader, &format!("{path}.DefaultValue"))?;
    let autogenerated_default_value =
        read_blueprint_pin_string(reader, &format!("{path}.AutogeneratedDefaultValue"))?;
    let default_object = read_package_index(reader, &format!("{path}.DefaultObject"))?;
    let default_text_value = read_ue57_text(reader, &format!("{path}.DefaultTextValue"))?;
    let linked_to = read_ed_graph_pin_references(reader, &format!("{path}.LinkedTo"))?;
    let sub_pins = read_ed_graph_pin_references(reader, &format!("{path}.SubPins"))?;
    let parent_pin = read_ed_graph_pin_reference(reader, &format!("{path}.ParentPin"))?;
    let reference_pass_through_connection =
        read_ed_graph_pin_reference(reader, &format!("{path}.ReferencePassThroughConnection"))?;
    let persistent_guid = reader.read_guid(&format!("{path}.PersistentGuid"))?;
    let flags = reader.read_u32(&format!("{path}.Flags"))?;
...
fn read_blueprint_pin_string(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<String, AssetError> {
    // Current editor packages may prefix a native pin FString with an i32 -1 encoding marker.
    // A direct FString also occurs, including negative UTF-16 lengths. The otherwise-empty direct
    // UTF-16 length -1 is not emitted for a useful string, so it is unambiguous as the envelope.
    let mut probe = reader.clone();
    if probe.read_i32(&format!("{path}.Encoding"))? == -1 {
        *reader = probe;
    }
    Ok(reader.read_fstring(path)?)
}

fn read_ed_graph_pin_type(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<EdGraphPinType, AssetError> {
    let category = reader.read_name_ref(&format!("{path}.PinCategory"))?;
    let subcategory = reader.read_name_ref(&format!("{path}.PinSubCategory"))?;
    let subcategory_object = read_package_index(reader, &format!("{path}.PinSubCategoryObject"))?;
    let container_type = match reader.read_u8(&format!("{path}.ContainerType"))? {
        0 => EdGraphPinContainerType::None,
        1 => EdGraphPinContainerType::Array,
        2 => EdGraphPinContainerType::Set,
        3 => EdGraphPinContainerType::Map,
        value => {
            return Err(AssetError::new(
                AssetErrorKind::MalformedData,
                format!("{path}.ContainerType has invalid UE 5.7 value {value}"),
            ));
        }
    };
    let value_type = if container_type == EdGraphPinContainerType::Map {
        Some(read_ed_graph_terminal_type(
            reader,
            &format!("{path}.PinValueType"),
        )?)
    } else {
        None
    };
    let is_reference = read_archive_bool(reader, &format!("{path}.IsReference"))?;
    let is_weak_pointer = read_archive_bool(reader, &format!("{path}.IsWeakPointer"))?;
    let member_reference = EdGraphMemberReference {
        parent: read_package_index(
            reader,
            &format!("{path}.PinSubCategoryMemberReference.MemberParent"),
        )?,
        name: reader.read_name_ref(&format!("{path}.PinSubCategoryMemberReference.MemberName"))?,
        guid: reader.read_guid(&format!("{path}.PinSubCategoryMemberReference.MemberGuid"))?,
    };
    let is_const = read_archive_bool(reader, &format!("{path}.IsConst"))?;
    let is_uobject_wrapper = read_archive_bool(reader, &format!("{path}.IsUObjectWrapper"))?;
    let serialize_as_single_precision_float =
        read_archive_bool(reader, &format!("{path}.SerializeAsSinglePrecisionFloat"))?;
...
fn read_ed_graph_terminal_type(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<EdGraphTerminalType, AssetError> {
    Ok(EdGraphTerminalType {
        category: reader.read_name_ref(&format!("{path}.TerminalCategory"))?,
        subcategory: reader.read_name_ref(&format!("{path}.TerminalSubCategory"))?,
        subcategory_object: read_package_index(
            reader,
            &format!("{path}.TerminalSubCategoryObject"),
        )?,
        is_const: read_archive_bool(reader, &format!("{path}.IsConst"))?,
        is_weak_pointer: read_archive_bool(reader, &format!("{path}.IsWeakPointer"))?,
        is_uobject_wrapper: read_archive_bool(reader, &format!("{path}.IsUObjectWrapper"))?,
    })
}

fn read_ed_graph_pin_references(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<Vec<Option<EdGraphPinReference>>, AssetError> {
    let count = reader.read_count(&format!("{path}.Count"))?;
    let capacity = reader.checked_vec_capacity::<Option<EdGraphPinReference>>(
        count,
        4,
        &format!("{path}.Count"),
    )?;
    ...
    for index in 0..count {
        references.push(read_ed_graph_pin_reference(
            reader,
            &format!("{path}[{index}]"),
        )?);
    }
...
fn read_ed_graph_pin_reference(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<Option<EdGraphPinReference>, AssetError> {
    if read_archive_bool(reader, &format!("{path}.IsNull"))? {
        return Ok(None);
    }
    Ok(Some(EdGraphPinReference {
        owning_node: read_package_index(reader, &format!("{path}.OwningNode"))?,
        pin_id: reader.read_guid(&format!("{path}.PinId"))?,
    }))
}
...
fn read_package_index(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<PackageIndex, AssetError> {
    Ok(PackageIndex::from_raw(reader.read_i32(path)?))
}

fn read_archive_bool(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
    ...
    match reader.read_u32(path)? {
        0 => Ok(false),
        1 => Ok(true),
        value => Err(AssetError::new(
            AssetErrorKind::MalformedData,
            format!("invalid archive bool {value} at byte {offset} while reading {path}"),
        )),
    }
}
...
fn read_ue57_text(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
) -> Result<TextValue, AssetError> {
    read_ue57_text_at_depth(reader, path, 0)
}

const MAX_TEXT_HISTORY_DEPTH: usize = 16;

fn read_ue57_text_at_depth(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
    depth: usize,
) -> Result<TextValue, AssetError> {
    if depth >= MAX_TEXT_HISTORY_DEPTH {
        return Err(AssetError::new(
            AssetErrorKind::ResourceLimit,
            format!("{path} exceeds FText history depth limit {MAX_TEXT_HISTORY_DEPTH}"),
        ));
    }

    let _flags = reader.read_i32(&format!("{path}.Flags"))?;
    match reader.read_i8(&format!("{path}.HistoryType"))? {
        -1 => {
            let culture_invariant = read_archive_bool(reader, &format!("{path}.CultureInvariant"))?;
            let source = if culture_invariant {
                reader.read_fstring(&format!("{path}.CultureInvariantString"))?
            } else {
                String::new()
            };
            Ok(TextValue {
        ...
        0 => {
            let namespace = reader.read_fstring(&format!("{path}.Namespace"))?;
            let key = reader.read_fstring(&format!("{path}.Key"))?;
            let source = reader.read_fstring(&format!("{path}.SourceString"))?;
            Ok(TextValue {
                source,
        ...
        1 => {
            let format = Box::new(read_ue57_text_at_depth(
                reader,
                &format!("{path}.FormatText"),
                depth + 1,
            )?);
            let arguments_path = format!("{path}.Arguments");
            let mut count = reader.read_count(&format!("{arguments_path}.Count"))?;
            if count == 0 {
                // Some saved editor text uses the same delta-map framing as a tagged map: an
                // empty keys-to-remove array followed by the actual named-argument count. Older
                // saves write the argument count directly. Probe the optional second count without
                // disturbing a genuinely empty argument map.
                let mut probe = reader.clone();
                if let Ok(entry_count) =
                    probe.read_count(&format!("{arguments_path}.Entries.Count"))
                    && entry_count > 0
                {
                    *reader = probe;
                    count = entry_count;
            ...
            let capacity = reader.checked_vec_capacity::<NamedTextFormatArgument>(
                count,
                5,
                &format!("{arguments_path}.Count"),
            )?;
            let mut arguments = Vec::with_capacity(capacity);
            for index in 0..count {
                let argument_path = format!("{arguments_path}[{index}]");
                let name = reader.read_fstring(&format!("{argument_path}.Name"))?;
                let value = read_text_format_argument(
                    reader,
                    &format!("{argument_path}.Value"),
                    depth + 1,
                )?;
                arguments.push(NamedTextFormatArgument { name, value });
            }
            let source = render_named_text_format(&format.source, &arguments);
            Ok(TextValue {
                source,
                history: TextHistory::NamedFormat { format, arguments },
            })
        }
        history_type => Err(AssetError::new(
            AssetErrorKind::UnsupportedCapability,
            format!("{path} uses unsupported UE 5.7 FText history {history_type}"),
        )),
    }
}

fn read_text_format_argument(
    reader: &mut crate::archive::Reader<'_>,
    path: &str,
    depth: usize,
) -> Result<TextFormatArgumentValue, AssetError> {
    match reader.read_i8(&format!("{path}.Type"))? {
        0 => Ok(TextFormatArgumentValue::Int(
            reader.read_i64(&format!("{path}.Int"))?,
        )),
        1 => Ok(TextFormatArgumentValue::UInt(
            reader.read_u64(&format!("{path}.UInt"))?,
        )),
        2 => Ok(TextFormatArgumentValue::Float(
            reader.read_f32(&format!("{path}.Float"))?,
        )),
        3 => Ok(TextFormatArgumentValue::Double(
            reader.read_f64(&format!("{path}.Double"))?,
        )),
        4 => Ok(TextFormatArgumentValue::Text(Box::new(
            read_ue57_text_at_depth(reader, &format!("{path}.Text"), depth)?,
        ))),
        5 => Ok(TextFormatArgumentValue::Gender(
            reader.read_u64(&format!("{path}.Gender"))?,
        )),
        argument_type => Err(AssetError::new(
            AssetErrorKind::UnsupportedCapability,
            format!("{path} uses unsupported FText format argument type {argument_type}"),
        )),
    }
}

fn render_named_text_format(format: &str, arguments: &[NamedTextFormatArgument]) -> String {
    arguments
        .iter()
        .fold(format.to_owned(), |rendered, argument| {
            rendered.replace(
                &format!("{{{}}}", argument.name),
                &render_text_format_argument(&argument.value),
            )
        })
}

fn render_text_format_argument(value: &TextFormatArgumentValue) -> String {
    match value {
        TextFormatArgumentValue::Int(value) => value.to_string(),
        TextFormatArgumentValue::UInt(value) | TextFormatArgumentValue::Gender(value) => {
            value.to_string()
        }
        TextFormatArgumentValue::Float(value) => value.to_string(),
        TextFormatArgumentValue::Double(value) => value.to_string(),
        TextFormatArgumentValue::Text(value) => value.source.clone(),
    }
}
...
mod tests {
    ...
    #[test]
    fn reads_culture_invariant_ftext_source() {
        let mut bytes = Vec::new();
        push_i32(&mut bytes, 2); // flags
        bytes.push(u8::MAX); // history type None
        push_i32(&mut bytes, 1); // has culture-invariant string
        push_fstring(&mut bytes, "X");

        let mut reader = Reader::new(&bytes);
        let text = read_ue57_text(&mut reader, "FriendlyName").unwrap();
        assert_eq!(text.source, "X");
        assert_eq!(text.history, TextHistory::None);
        assert_eq!(reader.remaining(), 0);
    }

    #[test]
    fn reads_and_renders_named_format_ftext() {
        fn push_base_text(bytes: &mut Vec<u8>, source: &str) {
            push_i32(bytes, 0); // flags
            bytes.push(0); // history type Base
            push_fstring(bytes, ""); // namespace
            push_fstring(bytes, ""); // key
            push_fstring(bytes, source);
        }

        for uses_delta_map_framing in [false, true] {
            let mut bytes = Vec::new();
            push_i32(&mut bytes, 0); // flags
            bytes.push(1); // history type NamedFormat
            push_i32(&mut bytes, 8); // format-text flags
            bytes.push(0); // format-text history type Base
            push_fstring(&mut bytes, "KismetSchema");
            push_fstring(&mut bytes, "SplitPinFriendlyNameFormat");
            push_fstring(&mut bytes, "{PinDisplayName} {ProtoPinDisplayName}");
            if uses_delta_map_framing {
                push_i32(&mut bytes, 0); // keys to remove
            }
            push_i32(&mut bytes, 2); // named arguments
            push_fstring(&mut bytes, "PinDisplayName");
            bytes.push(4); // argument type Text
            push_base_text(&mut bytes, "Return Value");
            push_fstring(&mut bytes, "ProtoPinDisplayName");
            bytes.push(4); // argument type Text
            push_base_text(&mut bytes, "X");

            let mut reader = Reader::new(&bytes);
            let text = read_ue57_text(&mut reader, "FriendlyName").unwrap();
            assert_eq!(text.source, "Return Value X");
            let TextHistory::NamedFormat { format, arguments } = text.history else {
                panic!("expected named-format history");
            };
            assert_eq!(format.source, "{PinDisplayName} {ProtoPinDisplayName}");
    ...
    #[test]
    fn reads_direct_and_enveloped_blueprint_pin_strings() {
        let mut bytes = Vec::new();
        push_fstring(&mut bytes, "Direct");
        push_i32(&mut bytes, -1); // encoding envelope
        push_fstring(&mut bytes, "Enveloped");
        push_i32(&mut bytes, -2); // direct UTF-16 FString
        ...
        let mut reader = Reader::new(&bytes);
        assert_eq!(
            read_blueprint_pin_string(&mut reader, "Direct").unwrap(),
            "Direct"
        );
        assert_eq!(
            read_blueprint_pin_string(&mut reader, "Enveloped").unwrap(),
            "Enveloped"
        );
        assert_eq!(
            read_blueprint_pin_string(&mut reader, "Wide").unwrap(),
            "猫"
        );
        assert_eq!(reader.remaining(), 0);
    }
    ...
    #[test]
    fn recognizes_blueprint_k2_and_comment_node_classes() {
        assert!(is_blueprint_graph_node_class(
            "/Script/BlueprintGraph.K2Node_CallFunction"
        ));
        assert!(is_blueprint_graph_node_class(
            "/Script/Engine.EdGraphNode_Comment"
        ));
        assert!(!is_blueprint_graph_node_class("/Script/Engine.Blueprint"));
    }

    #[test]
    fn accepts_only_the_observed_ue57_blueprint_saved_revisions() {
        for ue5 in UE57_BLUEPRINT_PACKAGE_VERSIONS {
            let versions = crate::version::VersionContext {
                ue4: crate::version::VersionContext::LATEST_SUPPORTED_UE4,
                ue5: *ue5,
                ..crate::version::VersionContext::default()
            };
            assert!(supports_blueprint_graph_package_version(&versions));
        }

        for (ue4, ue5) in [(521, 1018), (522, 1009), (522, 1016), (522, 1019)] {
            let versions = crate::version::VersionContext {
                ue4,
                ue5,
                ..crate::version::VersionContext::default()
