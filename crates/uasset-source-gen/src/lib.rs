...
pub fn generate(
    ...
    let mut native_aliases = BTreeMap::new();
    let mut native_layouts = BTreeMap::new();
    ...
    for module in &config.modules {
        for relative_path in &module.headers {
            ...
            native_sources.insert(relative_path.clone(), tokens.clone());
            native_layouts::collect_aliases(&tokens, &mut native_aliases);
            reflected.extend(
    ...
    if has_serialization_fact(&serializer_facts, SerializationFact::EnumData) {
        ...
        native_layouts.insert("UEnum".to_owned(), native_layouts::enumeration());
    }
    ...
    if has_serialization_fact(&serializer_facts, SerializationFact::StringTableData) {
        ...
        native_layouts.insert(
            "FStringTable".to_owned(),
            native_layouts::string_table(&native_aliases)?,
        );
    }
...
fn validate_skeleton_serializer(
    ...
    diagnostics.push(SourceDiagnostic {
        path: serializer.path.clone(),
        line: serializer.line,
        message: "recognized FReferenceSkeleton bone-info prefix; reference poses and name indices use the shared native layout; later data remain compatibility-opaque"
            .to_owned(),
    });
    Ok(())
}
