...
fn run() -> Result<(), String> {
    ...
    if command == "check" {
        ...
        if !matches_generated_json(&existing, &json) {
            return Err(format!(
...
fn matches_generated_json(existing: &str, generated: &str) -> bool {
    // Git may check generated text out with CRLF on Windows. Keep every other byte
    // significant so content, field order, and canonical formatting still detect drift.
    existing.replace("\r\n", "\n") == generated
}
...
#[cfg(test)]
mod tests {
    use super::{generated_json, matches_generated_json};
    use uasset_parser::schema::{SOURCE_MODEL_SCHEMA_VERSION, SourceModel};

    #[test]
    fn freshness_accepts_windows_checkouts_but_rejects_model_changes() {
        let generated = "{\n\t\"schema_version\": 9,\n\t\"value\": \"escaped\\r\\ntext\"\n}\n";
        assert!(matches_generated_json(generated, generated));
        assert!(matches_generated_json(
            &generated.replace('\n', "\r\n"),
            generated
        ));
        for stale in [
            generated.replace(": 9", ": 8"),
            generated.replace("\\r\\n", "\\n"),
            generated.replace('\t', "  "),
        ] {
            assert!(!matches_generated_json(&stale, generated));
        }
    }
