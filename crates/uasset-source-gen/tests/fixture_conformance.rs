...
fn assert_value_has_no_raw(value: &PropertyValue, path: &str) {
    match value {
        ...
        PropertyValue::NativeStruct { fields } => {
            for field in fields {
                assert_value_has_no_raw(&field.value, path);
            }
        }
...
fn generated_and_legacy_string_table_semantics_are_identical() {
    ...
    let evidence: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(workspace_path(
            "fixtures/unreal-project/FixtureExpected/parser-targets/string-table.json",
        ))
        .unwrap(),
    )
    .unwrap();
    assert_eq!(generated.namespace, evidence["namespace"].as_str().unwrap());
    assert_eq!(
        serde_json::to_value(&generated.metadata).unwrap(),
        evidence["metadata"]
    );
    assert_eq!(generated.metadata.len(), 2);
    assert_eq!(
        generated.metadata["StatusSaving"]["Comment"],
        "Progress: café / 保存"
    );
    assert_eq!(generated.metadata["StatusSaving"]["Empty"], "");
}
