...
| Native DataTable row-handle decode, Apply, Save, and saved reload            | unreal-tested  | `pnpm run test:unreal-authoring`; the mutation matrix changes `DT_LeftReferences` and compares the independently decoded saved fingerprint                                                                                              |
| Cross-table row-reference resolution and broken-target diagnostics           | adapter-tested | `pnpm exec vitest run packages/authoring/src/relationships.test.ts apps/cli/src/index.e2e.test.ts`; the public CLI resolves both fixture references                                                                                     |
| Read-only joined projection with source and target provenance                | adapter-tested | `pnpm exec vitest run packages/authoring/src/joined-views.test.ts apps/cli/src/index.e2e.test.ts`; unresolved source rows remain visible                                                                                                |
| Relationship matrix table visibility and isolation                           | product-tested | `pnpm exec vitest run extensions/data-authoring/src/authoring-combined-view.component.test.tsx`; visibility never changes canonical evidence                                                                                            |
| Shape-aware table charts from bool, enum, and numeric fields                 | adapter-tested | `pnpm exec vitest run packages/authoring/src/analysis.test.ts apps/cli/src/index.e2e.test.ts extensions/data-authoring/src/authoring-analysis-view.component.test.tsx extensions/data-authoring/src/authoring-route.component.test.tsx` |
