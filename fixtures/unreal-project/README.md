...
`/Game/Fixture/Blueprints/BP_GraphFixture` is a UE 5.7 Actor Blueprint with a saved BeginPlay event
linked to a function call. It protects the portable graph projection's node, position, pin, default,
and link decoding without relying on a studio project. The commandlet's `-BlueprintOnly` flag
regenerates just this package.
