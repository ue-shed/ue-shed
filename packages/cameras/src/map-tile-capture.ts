...
function runMapCaptureWith(args: {
	...
	return Effect.gen(function* () {
		...
		const runId = args.options.runId ?? randomUUID();
		const correlationId = args.options.correlationId ?? runId;
		const startedAt = isoNow(yield* Clock.currentTimeMillis);
		const root = mapCaptureRoot(args.options.projectRoot);
		const stagingRoot = join(root, `.staging-${runId}`);
		const finalRoot = join(mapCaptureRunsRoot(args.options.projectRoot, plan.id), runId);
		const attemptRoot = join(mapCaptureAttemptsRoot(args.options.projectRoot, plan.id), runId);
		...
		let cancelled = false;
		for (const [batchIndex, batch] of batches(keys, maximumTilesPerRequest).entries()) {
			const request = makeRequest({
				batch,
				correlationId,
				grid: inspected.grid,
				operationId: `${runId}-${batchIndex}`,
				plan,
				runId
			});
			...
			for (const key of batch) {
					...
					failures.push({
						failure:
							...
							captureFailure({
								...
								message: "Editor response did not inventory the requested tile.",
								retrySafe: false
							}),
						key
					});
				...
				if (
					normalizedSource === normalizedStagingRoot ||
					!normalizedSource.startsWith(`${normalizedStagingRoot}\\`)
				) {
					failures.push({
						failure: captureFailure({
							code: "write_failed",
				...
				const dimensions = yield* Effect.try({
					try: () => readPngDimensions(bytes),
						...
						new MapCaptureRunError({
							message: String(cause),
							operation: "capture",
							recovery: "Quarantine the invalid staged artifact and retry its tile.",
							runId
						})
				});
				if (
			...
			Effect.mapError(
				(cause) =>
					new MapCaptureRunError({
						message: String(cause),
						operation: "publish",
						recovery: "Inspect grid and artifact inventory invariants before retrying.",
						runId
					})
			)
		);
		if (manifest.state === "complete") {
			...
			return { manifest, manifestPath: join(finalRoot, "manifest.json"), published: true };
		}
		yield* args.repository
