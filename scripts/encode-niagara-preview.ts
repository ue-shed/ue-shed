...
import { snapshotNiagaraFrames } from "../packages/niagara/src/verified-frames.ts";
import {
...
try {
	const framesRoot = await snapshotNiagaraFrames(root, staging, manifest.artifacts);
	...
	const input = [
		...
		"-i",
		join(framesRoot, "frame_%04d.png")
	];
	const composite =
	...
	await copyFile(join(staging, poster.relativePath), join(staging, "poster.png"));
	const posterSha256 = hash(await readFile(join(staging, "poster.png")));
	if (posterSha256 !== poster.sha256) throw new Error("Poster integrity failed.");
	...
	await rm(framesRoot, { recursive: true });
