...
type AgentName = keyof typeof defaultModels;
...
interface AgentDefinition {
	args: (input: AgentArguments) => string[];
	command: string;
...
interface AgentExecution extends Omit<CommandResult, "status"> {
...
function readFlag(args: string[], index: number) {
...
function run(command: string, args: string[], runOptions: RunOptions = {}): CommandResult {
...
async function runAgent({
	...
	return {
		error: timedOut ? `timed out after ${timeoutMs / 1000} seconds` : spawnErrorMessage,
...
function runPnpm(args: string[], runOptions: RunOptions = {}): CommandResult {
...
async function prepareSourceKit(manifest: AdoptionManifest, sourceCommit: string): Promise<void> {
...
function adoptionPrompt(target: string, sourceCommit: string): string {
...
function parseAgentMetrics(stdout: string): AgentMetrics {
	const metrics: AgentMetrics = { toolCalls: 0, tools: {} };
...
async function materializeBaseline(sourceCommit: string): Promise<string> {
...
async function fileDigestMap(root: string): Promise<Map<string, string>> {
...
async function compareWithBaseline(target: string, baseline: string) {
...
async function verifyTarget(label: string, target: string, sourceCommit: string, baseline: string) {
	...
	let reportWordCount: number | undefined;
	try {
