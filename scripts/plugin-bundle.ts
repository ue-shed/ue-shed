...
interface BuildPluginBundleOptions {
	...
	readonly candidateManifest?: string | undefined;
...
function validateReleaseVersion(value: string) {
...
function validateCommit(value: string | null | undefined) {
...
function relativePluginPath(pluginRoot: string, path: string) {
...
function sortArchivePaths(paths: readonly string[]) {
	return [...paths].sort((left, right) => (left < right ? -1 : left > right ? 1 : 0));
}

async function prepareArchiveTree({
	pluginRoot,
	sourceFiles,
	licensePath,
	stageRoot
...
function parseArguments(args: readonly string[]) {
	if (args[0] !== "bundle") {
		throw new Error(
			"Usage: node scripts/plugin-bundle.ts bundle --version <semver> --output <empty-dir> [options]"
		);
	}
