...
export function parseRegistryIntegrity({ packageSpec, status, stdout = "", stderr = "" }) {
	if (status === 0) {
	...
	if (errorPayload?.error?.code === "E404" || /(?:^|\s)E404(?:\s|$)/u.test(combined)) {
		return null;
	}
	throw new Error(
...
export function publicationDecision({ packageSpec, localIntegrity, registryIntegrity }) {
...
export function validatePublicationManifest({ manifest, expectedVersion }) {
	const failures = [];
...
function publishTarball({ tarball, tag, provenance }) {
	const args = [
		"publish",
		tarball,
		"--access",
...
function parseArguments(args) {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (!key?.startsWith("--") || args[index + 1] === undefined) {
			throw new Error(
				"Usage: node scripts/publish-public-packages.mjs --manifest <path> " +
