interface TestGate {
	...
	readonly missing: (environment: NodeJS.ProcessEnv) => string | undefined;
}
...
function selectedGates(arguments_: readonly string[]) {
...
export function reportUnrealTestGates(
...
export function reportPerforceMapHistoryTestGate(
	environment: NodeJS.ProcessEnv,
	arguments_: readonly string[] = []
) {
	if (arguments_.includes("component")) return;
