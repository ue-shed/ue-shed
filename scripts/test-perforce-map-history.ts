...
type FixtureAction = "add" | "delete" | "edit";
...
interface SeededFixture {
	conventional: {
		...
		revisions: SubmittedChange[];
	};
	relocatedConventional: {
		baseline: SubmittedChange;
		mapPath: string;
		relocatedMapPath: string;
		range: { since: string; until: string };
		revisions: SubmittedChange[];
	};
	worldPartition: {
...
interface PerforceContext {
	environment: NodeJS.ProcessEnv;
	p4: string;
	workspace: string;
}

export interface PerforceMapHistoryFixture {
	environment: NodeJS.ProcessEnv;
...
function p4Environment(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
...
async function cachePinnedBinary(directory: string, binary: PinnedBinary): Promise<string> {
...
async function resolveBinaries(): Promise<{ p4: string; p4d: string }> {
...
function workspacePath(workspace: string, projectRelativePath: string): string {
...
async function submitFixtureRevision({
	p4,
	environment,
	revision,
	workspace
}: PerforceContext & { revision: FixtureRevision }): Promise<SubmittedChange> {
	for (const file of revision.files) {
...
async function advancePast(second: number): Promise<void> {
...
async function seedFixture({
	...
	environment,
	workspace
}: PerforceContext): Promise<SeededFixture> {
	...
	const worldPartitionRevisions: SubmittedChange[] = [];
...
async function runCliJourney(fixture: PerforceMapHistoryFixture): Promise<void> {
 ...
 */
export async function startPerforceMapHistoryFixture(): Promise<PerforceMapHistoryFixture> {
	...
	try {
		...
		const configurationServer = server;
		configurationServer.kill();
		await waitForProcessExit(configurationServer);
