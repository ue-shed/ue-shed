...
try {
	...
	await writeFile(
		...
		`${[
			...
			"      targetIds: [target.id]",
			"    });",
			"    yield* Effect.promise(() => writeFile(resolve('./custodian-root/PackedFixture/Intermediate/after-review.bin'), 'changed-after-review'));",
			...
			"if (custodian.status !== 'refused' || !['proposal_stale', 'editor_running'].includes(custodian.refusal?.code)) {",
			"  throw new Error(`packed Custodian did not safely refuse trash proposal: ${JSON.stringify(custodian)}`);",
			"}",
