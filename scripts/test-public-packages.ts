...
try {
	...
	await writeFile(
		...
		`${[
			...
			"import { makeWorldPreparation, withPreparedWorld } from '@ue-shed/world';",
			"import { WorldRequest } from '@ue-shed/world/browser';",
			"if (typeof withPreparedWorld !== 'function') throw new Error('bad world scope export');",
			"const worldClient = makeWorldPreparation({ request: () => Effect.succeed({ schemaVersion: 1, producerKind: 'unreal_editor', capabilities: [] }) }, 'http://fixture');",
			"const unavailableWorld = await Effect.runPromise(worldClient.describe().pipe(Effect.result));",
			"if (unavailableWorld._tag !== 'Failure' || unavailableWorld.failure.code !== 'capability_unavailable') throw new Error('world capability negotiation failed');",
			"Schema.decodeUnknownSync(WorldRequest)({ contract: { name: 'ue-shed-world-preparation', version: { major: 1, minor: 0 } }, action: 'describe' });",
