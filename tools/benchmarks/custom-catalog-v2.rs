...
const CATALOG_DIRECTORY: &str = "catalogs-custom-research-v2";
...
mod tests {
    ...
    fn writer_exclusion_survives_clear_and_readers_cannot_mutate() {
        ...
        assert!(CustomCatalog::open(&root, &fixture_project_id()).is_err());
        let mut reader = CustomCatalog::open_for_query(&root, &fixture_project_id()).unwrap();
        ...
        assert!(CustomCatalog::open(&root, &fixture_project_id()).is_ok());
    }
    ...
    fn failed_writes_and_sync_preserve_the_previous_generation() {
        ...
        for point in [
            ...
            let reader = CustomCatalog::open_for_query(
    ...
    fn damaged_payload_is_reported_to_queries_and_quarantined_by_refresh() {
        ...
        *bytes.last_mut().unwrap() ^= 0x40;
        ...
        let mut recovered = CustomCatalog::open(&root, &fixture_project_id()).unwrap();
    ...
    fn crash_writer_child() {
        ...
        let mut writer = CustomCatalog::open(&root, &fixture_project_id()).unwrap();
        let mut scanner = refresh_fixture();
        scanner.entries[0].modified_nanos += 1;
        refresh(
            &mut writer,
            ...
            for point in [
                ...
                let mut child = Child(
                    std::process::Command::new(std::env::current_exe().unwrap())
                        .args([
                            "--exact",
                            "direct_executor::catalog_custom::tests::crash_writer_child",
                            "--ignored",
                        ])
