...
{
	FPendingMapTileEncode(const TSharedRef<FJsonObject> &InResult, FString InPath, double InStartedSeconds,
						  TFuture<bool> &&InWritten)
		: Result(InResult), Path(MoveTemp(InPath)), StartedSeconds(InStartedSeconds),
...
{
	FPendingMapTileReadback(FImage &&InImage, const TSharedRef<FJsonObject> &InResult, FString InPath,
							double InStartedSeconds, const FIntRect &InCrop, int32 InOutputSize)
		: Image(MoveTemp(InImage)), Result(InResult), Path(MoveTemp(InPath)),
		  StartedSeconds(InStartedSeconds), Crop(InCrop), OutputSize(InOutputSize)
	{
	}
...
TSharedRef<FJsonObject> MapTileFailure(const TCHAR *Code, const TCHAR *Message, const TCHAR *Recovery,
									   bool bRetrySafe)
{
...
void SetMapTileCounts(const TSharedRef<FJsonObject> &Result, int32 Requested, int32 Succeeded, int32 Failed)
{
...
bool ReadInteger(const TSharedPtr<FJsonObject> &Object, const TCHAR *Field, int32 &OutValue)
{
	double Value;
	if (!Object->TryGetNumberField(Field, Value) || Value != FMath::FloorToDouble(Value) ||
...
bool CaptureViewportHighResolutionLevel(const TArray<TSharedPtr<FJsonValue>> &TileValues, int32 TilePixelSize,
										int32 GutterPixels, double CaptureZ, bool bFog, bool bVolumetricFog,
										...
										FString &OutFailureRecovery)
{
	...
	{
		return Fail(TEXT("capture_failed"),
	...
	for (const TSharedPtr<FJsonValue> &TileValue : TileValues)
	{
		...
		const bool bValid =
			TileValue.IsValid() && TileValue->TryGetObject(TileObject) &&
			(*TileObject)->TryGetObjectField(TEXT("key"), Key) &&
			ReadInteger(*Key, TEXT("zoom"), Tile.Zoom) && ReadInteger(*Key, TEXT("row"), Tile.Row) &&
			ReadInteger(*Key, TEXT("column"), Tile.Column) && Tile.Zoom >= 0 && Tile.Row >= 0 &&
			Tile.Column >= 0 && (*TileObject)->TryGetNumberField(TEXT("unitsPerPixel"), TileUnitsPerPixel) &&
			FMath::IsFinite(TileUnitsPerPixel) && TileUnitsPerPixel > 0.0 &&
		...
		const FString KeyIdentity = FString::Printf(TEXT("%d/%d/%d"), Tile.Zoom, Tile.Row, Tile.Column);
		if (!bValid || (Zoom != INDEX_NONE && Tile.Zoom != Zoom) ||
			(UnitsPerPixel > 0.0 &&
		...
		Tile.Result->SetObjectField(TEXT("key"), MapTileKeyJson(Tile.Zoom, Tile.Row, Tile.Column));
	...
	if (RenderProfile == TEXT("observation"))
		RendererPolicy->SetStringField(TEXT("profile"), TEXT("observation"));
	...
	const auto Frame = Renderer->RenderBlocking(UEShedRenderFrame(
		SessionId, TEXT("whole-level"),
		UEShedCameraPose(FVector((FullMinX + FullMaxX) * .5, (FullMinY + FullMaxY) * .5, CaptureZ),
						 FRotator(-90, 0, 0), FullMaxY - FullMinY + GutterPixels * 2.0 * UnitsPerPixel, true),
		RenderWidth, RenderHeight));
	if (Frame->GetStringField(TEXT("status")) != TEXT("captured"))
		return Fail(*Frame->GetStringField(TEXT("code")), *Frame->GetStringField(TEXT("message")),
					TEXT("Reduce the whole-level size or inspect renderer readiness."));
	const FString ScreenshotPath =
		FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UEShed/CameraRenderStaging"),
						Frame->GetObjectField(TEXT("artifact"))->GetStringField(TEXT("relativePath")));
	FImage FullImage;
	if (!FImageUtils::LoadImage(*ScreenshotPath, FullImage))
		return Fail(TEXT("capture_failed"), TEXT("The shared renderer image could not be read."),
					TEXT("Inspect native staging."));
	const auto Closed = Renderer->Close();
	if (Closed->GetStringField(TEXT("status")) != TEXT("closed"))
		return Fail(TEXT("capture_failed"), TEXT("The shared renderer could not restore the viewport."),
					TEXT("Inspect editor ownership before retrying."));
	...
	const double CaptureDurationMs = (FPlatformTime::Seconds() - CaptureStartedSeconds) * 1000.0;
	for (FViewportHighResolutionTile &Tile : Tiles)
	{
		FImage TileImage(TilePixelSize, TilePixelSize, ERawImageFormat::BGRA8, EGammaSpace::sRGB);
		...
		{
			// The shared absolute camera already uses the manifest's +X-up/+Y-right orientation.
			const uint8 *SourceRow = FullImage.RawData.GetData() + (SourceY + PixelRow) * SourceStride;
			uint8 *Destination = TileImage.RawData.GetData() + PixelRow * TileImage.GetStrideBytes();
			for (int32 PixelColumn = 0; PixelColumn < TilePixelSize; ++PixelColumn)
			{
				const uint8 *Source = SourceRow + (SourceX + PixelColumn) * BytesPerPixel;
				FMemory::Memcpy(Destination + PixelColumn * BytesPerPixel, Source, BytesPerPixel);
			}
		}
		const FString CapturePath =
			FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UEShed"), TEXT("MapTileStaging"), RunId,
		...
		if (!FUEShedTransientCapture::WritePng(CapturePath, TileImage))
		{
			...
			Tile.Result->SetObjectField(
				TEXT("failure"),
				MapTileFailure(TEXT("encoding_failed"),
							   TEXT("Unreal could not encode a tile cut from the viewport screenshot."),
							   TEXT("Check the project Saved directory and retry this zoom."), true));
			OutFailed += 1;
		}
		else
		{
			Tile.Result->SetStringField(TEXT("status"), TEXT("captured"));
			Tile.Result->SetStringField(TEXT("stagedPath"), FPaths::ConvertRelativePathToFull(CapturePath));
			Tile.Result->SetNumberField(TEXT("width"), TilePixelSize);
			Tile.Result->SetNumberField(TEXT("height"), TilePixelSize);
			Tile.Result->SetNumberField(TEXT("bytes"), IFileManager::Get().FileSize(*CapturePath));
			Tile.Result->SetNumberField(TEXT("captureDurationMs"), CaptureDurationMs);
			Tile.Result->SetObjectField(TEXT("renderEvidence"), Frame->GetObjectField(TEXT("evidence")));
			OutSucceeded += 1;
		}
...
static void CaptureMapTilesInternal(const FString &RequestJson, FString &ResultJson, bool bAllowAsync)
{
	...
	{
		Fail(TEXT("invalid_request"), TEXT("Map tile capture request exceeds 256 KiB."),
			 TEXT("Send a bounded batch of at most 64 tiles."), false);
		return;
	}
	...
	{
		Fail(TEXT("invalid_request"), TEXT("Map tile capture request is not valid JSON."),
			 TEXT("Validate the request against ue-shed-map-tile-capture 1.0."), false);
		return;
	}
	...
	if (!IsMapTileSafeIdentifier(RequestedOperationId) || !IsMapTileSafeIdentifier(RequestedCorrelationId) ||
		!IsMapTileSafeIdentifier(PlanId) || !IsMapTileSafeIdentifier(RunId))
	{
		Fail(TEXT("invalid_request"), TEXT("Map capture identities are invalid."),
			 TEXT("Use safe identifiers no longer than 128 characters."), false);
		return;
	}
	...
	if (GEditor == nullptr)
	{
		Fail(TEXT("capture_failed"), TEXT("The Unreal editor is unavailable."),
			 TEXT("Run map capture in an editor process."), true);
		return;
	}
	UWorld *World = GEditor->GetEditorWorldContext().World();
	if (World == nullptr)
	{
		Fail(TEXT("capture_failed"), TEXT("No editor world is open."),
			 TEXT("Open the Map Capture Plan's expected map and retry."), true);
		return;
	}
	...
	{
		Fail(TEXT("map_mismatch"), TEXT("The open editor map does not match the Map Capture Plan."),
			 TEXT("Open the expected map without discarding unsaved work, then retry."), true);
		return;
	}

	int32 TilePixelSize;
	int32 GutterPixels;
	if (!ReadInteger(Request, TEXT("tilePixelSize"), TilePixelSize) || TilePixelSize < 64 ||
		TilePixelSize > 4096 || !ReadInteger(Request, TEXT("gutterPixels"), GutterPixels) ||
		GutterPixels < 0 || GutterPixels > 32)
	{
		Fail(TEXT("invalid_request"), TEXT("Tile or gutter pixel size is outside v1 limits."),
			 TEXT("Use tile pixels 64-4096 and gutter pixels 0-32."), false);
		return;
	}
	...
	if (!Request->TryGetObjectField(TEXT("capture"), CapturePolicy) ||
		...
		!(*CapturePolicy)->TryGetNumberField(TEXT("z"), CaptureZ) || !FMath::IsFinite(CaptureZ) ||
		Pitch != -90.0 || Yaw != 0.0 || Roll != 0.0 ||
		(RenderProfile != TEXT("full_fidelity") && RenderProfile != TEXT("seam_stable") &&
		 RenderProfile != TEXT("scene_capture_defaults") && RenderProfile != TEXT("observation")))
	{
		Fail(TEXT("invalid_request"), TEXT("The top-down capture policy is invalid."),
			 TEXT("Use finite capture Z, orientation (-90,0,0), and a supported render profile."), false);
		return;
	}
	if (DataLayerMode != TEXT("unchanged"))
	{
		Fail(TEXT("data_layer_policy_unsupported"),
			 TEXT("Explicit Data Layer state is not supported by map tile capture v1."),
			 TEXT("Use unchanged Data Layers or install a project adapter with scoped restoration."), false);
		return;
	}
	...
	if (LodPolicy == TEXT("per_level_distance_scale"))
	{
		if (!(*Render)->TryGetArrayField(TEXT("lodDistanceScaleByZoom"), LodDistanceScales) ||
			LodDistanceScales->IsEmpty() || LodDistanceScales->Num() > 24)
		{
			Fail(TEXT("invalid_request"), TEXT("Per-level LOD policy requires one to 24 distance scales."),
				 TEXT("Provide one finite 0.1-100 scale for every captured pyramid zoom."), false);
			return;
		}
	}
	else if (LodPolicy != TEXT("natural"))
	{
		Fail(TEXT("invalid_request"),
			 ...
			 TEXT("Use natural or per-level distance-scale LOD selection."), false);
		return;
	}
	...
	if (CaptureBackend != TEXT("scene_capture_tiles") && CaptureBackend != TEXT("lit_camera_tiles") &&
		CaptureBackend != TEXT("viewport_high_resolution"))
	{
		Fail(TEXT("invalid_request"), TEXT("The requested Map Capture backend is unsupported."),
			 TEXT("Use lit_camera_tiles, scene_capture_tiles or viewport_high_resolution."), false);
		return;
	}
	if (!Request->TryGetArrayField(TEXT("tiles"), Tiles) || Tiles->IsEmpty() || Tiles->Num() > 64)
	{
		Fail(TEXT("invalid_request"), TEXT("Map tile capture requires one to 64 tiles."),
			 TEXT("Split the plan into bounded tile batches."), false);
		return;
	}
	...
	{
		MapTileTopFailure(ResultJson, OperationId, CorrelationId, TEXT("streaming_not_ready"),
	...
	{
		Fail(TEXT("invalid_request"), TEXT("Manual exposure requires lit_camera_tiles."),
			 TEXT("Remove exposureEV100 or select lit_camera_tiles."), false);
		return;
	}
	...
	else
	{
			...
			if (!bWritten)
			{
				...
				Pending.Result->SetObjectField(
					...
					MapTileFailure(TEXT("encoding_failed"),
								   ...
								   TEXT("Check the project Saved directory and retry this tile."), true));
				Failed += 1;
			}
			else
			{
				Pending.Result->SetStringField(TEXT("status"), TEXT("captured"));
				Pending.Result->SetStringField(TEXT("stagedPath"),
											   FPaths::ConvertRelativePathToFull(Pending.Path));
				Pending.Result->SetNumberField(TEXT("width"), TilePixelSize);
				Pending.Result->SetNumberField(TEXT("height"), TilePixelSize);
				Pending.Result->SetNumberField(TEXT("bytes"), IFileManager::Get().FileSize(*Pending.Path));
				Pending.Result->SetNumberField(TEXT("captureDurationMs"),
		...
		auto FinalizeAllEncodes = [&]() {
			while (!PendingEncodes.IsEmpty())
				FinalizeOldestEncode();
		};
		auto FinalizeAllReadbacks = [&]() {
			for (FPendingMapTileReadback &Pending : PendingReadbacks)
			{
				FImage Image = MoveTemp(Pending.Image);
				if (Image.SizeX != Pending.OutputSize || Image.SizeY != Pending.OutputSize)
				{
					FImageCore::ResizeImageInPlace(Image, Pending.OutputSize, Pending.OutputSize,
				...
				PendingEncodes.Emplace(Pending.Result, Pending.Path, Pending.StartedSeconds,
									   MoveTemp(Written));
				if (PendingEncodes.Num() >= MaximumPendingTileEncodes)
					FinalizeOldestEncode();
			}
		...
		for (const TSharedPtr<FJsonValue> &TileValue : *Tiles)
		{
			...
			bool bValid = TileValue.IsValid() && TileValue->TryGetObject(Tile) &&
						  (*Tile)->TryGetObjectField(TEXT("key"), Key) &&
						  ReadInteger(*Key, TEXT("zoom"), Zoom) && ReadInteger(*Key, TEXT("row"), Row) &&
						  ReadInteger(*Key, TEXT("column"), Column) && Zoom >= 0 && Row >= 0 && Column >= 0 &&
						  (*Tile)->TryGetNumberField(TEXT("unitsPerPixel"), UnitsPerPixel) &&
						  FMath::IsFinite(UnitsPerPixel) && UnitsPerPixel > 0.0 &&
			...
			{
				bValid = LodDistanceScales != nullptr && LodDistanceScales->IsValidIndex(Zoom) &&
						 (*LodDistanceScales)[Zoom]->TryGetNumber(LodDistanceScale) &&
						 FMath::IsFinite(LodDistanceScale) && LodDistanceScale >= 0.1 &&
			...
			const double Tolerance = FMath::Max(0.001, ExpectedWorldSize * UE_DOUBLE_SMALL_NUMBER);
			if (UniqueKeys.Contains(KeyIdentity) || MaxX <= MinX || MaxY <= MinY ||
			...
			const int32 RenderScale = RenderProfile == TEXT("seam_stable") ? SeamStableRenderScale : 1;
			...
			if (!Renderer || RendererLod != LodDistanceScale)
			{
				if (Renderer && Renderer->Close()->GetStringField(TEXT("status")) != TEXT("closed"))
				{
					FinalizeAllReadbacks();
					FinalizeAllEncodes();
				...
				auto SessionRequest = UEShedLegacyRenderRequest(
				...
				P->GetObjectField(TEXT("settling"))
					->SetNumberField(TEXT("minimumFrames"), RenderProfile == TEXT("observation")
																? 1
																: FullFidelityWarmupCaptures + 1);
				TSharedPtr<FJsonObject> Error;
				Renderer = FUEShedCameraRenderSession::Open(SessionRequest, Error);
				RendererLod = LodDistanceScale;
				if (!Renderer)
				{
					FinalizeAllReadbacks();
					FinalizeAllEncodes();
			...
			const auto Frame = Renderer->RenderBlocking(UEShedRenderFrame(
				Renderer->Id(), FString::Printf(TEXT("tile-%d-%d-%d"), Zoom, Row, Column),
				UEShedCameraPose(Location, FRotator(-90, 0, 0), OrthoWidth, true), RenderSize, RenderSize));
			if (Frame->GetStringField(TEXT("status")) != TEXT("captured"))
			{
				TileResult->SetStringField(TEXT("status"), TEXT("failed"));
				TileResult->SetObjectField(
					TEXT("failure"),
					MapTileFailure(TEXT("capture_failed"), *Frame->GetStringField(TEXT("message")),
								   TEXT("Inspect shared rendering diagnostics."), true));
				TileResults.Add(MakeShared<FJsonValueObject>(TileResult));
				++Failed;
				continue;
			}
			TileResult->SetObjectField(TEXT("renderEvidence"), Frame->GetObjectField(TEXT("evidence")));
			FImage FullImage;
			const FString RawPath = FPaths::Combine(
				FPaths::ProjectSavedDir(), TEXT("UEShed/CameraRenderStaging"),
				Frame->GetObjectField(TEXT("artifact"))->GetStringField(TEXT("relativePath")));
			if (!FImageUtils::LoadImage(*RawPath, FullImage))
			{
				FinalizeAllReadbacks();
				FinalizeAllEncodes();
				MapTileTopFailure(ResultJson, OperationId, CorrelationId, TEXT("capture_failed"),
			...
			FullImage.ChangeFormat(ERawImageFormat::BGRA8, EGammaSpace::sRGB);
			FImage Cropped(TilePixelSize * RenderScale, TilePixelSize * RenderScale, ERawImageFormat::BGRA8,
						   EGammaSpace::sRGB);
			for (int32 Y = 0; Y < Cropped.SizeY; ++Y)
				FMemory::Memcpy(Cropped.RawData.GetData() + Y * Cropped.GetStrideBytes(),
								FullImage.RawData.GetData() +
									(Y + GutterPixels * RenderScale) * FullImage.GetStrideBytes() +
									GutterPixels * RenderScale * 4,
								Cropped.GetStrideBytes());
			const FString CapturePath = FPaths::Combine(
				FPaths::ProjectSavedDir(), TEXT("UEShed"), TEXT("MapTileStaging"), RunId,
				FString::Printf(TEXT("Z%02d"), Zoom), FString::Printf(TEXT("R%03d_C%03d.png"), Row, Column));
			const FIntRect Crop(GutterPixels * RenderScale, GutterPixels * RenderScale,
								(GutterPixels + TilePixelSize) * RenderScale,
								(GutterPixels + TilePixelSize) * RenderScale);
			PendingReadbacks.Emplace(MoveTemp(Cropped), TileResult, CapturePath, TileStartedSeconds, Crop,
									 TilePixelSize);
			if (PendingReadbacks.Num() >= MaximumPendingTileReadbacks)
				FinalizeAllReadbacks();
		}
		FinalizeAllReadbacks();
		FinalizeAllEncodes();
		...
		if (bCancelled)
		{
			for (int32 Index = TileResults.Num(); Index < Tiles->Num(); ++Index)
			{
				...
				TileResult->SetObjectField(
					TEXT("failure"),
					MapTileFailure(TEXT("cancelled"), TEXT("Editor shutdown cancelled map tile capture."),
								   TEXT("Restart the editor and retry the missing tile subset."), true));
				TileResults.Add(MakeShared<FJsonValueObject>(TileResult));
				Failed += 1;
			}
		}
	...
	if (bCancelled)
	{
		...
		Result->SetObjectField(TEXT("failure"),
							   MapTileFailure(TEXT("cancelled"),
											  TEXT("Map tile capture was cancelled during editor shutdown."),
											  TEXT("Retry only the missing or failed tiles."), true));
	}
	else if (bDirtyBefore != bDirtyAfter)
	{
		...
		Result->SetObjectField(
			TEXT("failure"),
			MapTileFailure(TEXT("dirty_state_changed"),
						   TEXT("The target map package dirty state changed during capture."),
						   TEXT("Inspect editor state; do not publish this attempt."), false));
	}
	else if (Failed == 0)
	{
